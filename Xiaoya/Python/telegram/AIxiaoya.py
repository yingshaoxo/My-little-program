...
global mulu
mulu=os.path.split( os.path.realpath( sys.argv[0] ) )[0]
dll=windll.LoadLibrary (mulu+"\\AIxiaoya.dll")
# Enable Logging
logging.basicConfig(
...
def message(bot, update, **kwargs):
    ...
    fuctnum=dll._eventGetMsg(c_char_p(bytes(str(update.message.chat_id),'utf-8')),c_char_p(bytes(update.message.text, 'utf-8')),c_char_p(bytes(token, 'utf-8')))
    if (fuctnum>=0):
        photolujin=mulu.replace('\\','/')+'/book/'+str(fuctnum)+'.png'
        bot.sendPhoto(update.message.chat_id, photo=open(photolujin, 'rb'))
        print ("sent："+photolujin)
